    ...
    <div class="relative min-h-screen left-0 w-full transition-all duration-500 ease-in-out z-0 top-0 p-4 sm:group-[.open]:w-[calc(100%_-_250px)] sm:group-[.open]:left-[250px] sm:left-[78px] sm:w-[calc(100%_-_78px)]">
        ...
        <div class="container mx-auto px-4 rounded-lg mt-4" style="background-color: #ffffff; height: 700px; border: 1px solid #acacac;">
            ...
            <div class="flex justify-end px-5 py-4 mt-4">
                <input type="text" placeholder="Search..." class="p-2 rounded-l-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div class="table-container" style="max-width: 100%; overflow-x: auto;">
                <table class="w-full border border-slate-600">
                    ...
                    <tbody>
                        @foreach($maps as $map)
                            <tr>
                                <td class="p-4 align-top"><img src="{{ asset('images/' . $map->mapImage) }}" alt="{{ $map->mapImage }}" style="max-width: 100px;"></td>
                                <td class="p-4 align-top"><img src="{{ asset('images/' . $map->spotImage) }}" alt="{{ $map->spotImage }}" style="max-width: 100px;"></td>
                                ...
                                <button class="p-2 bg-blue-500 text-white rounded-r-lg border" style="background-color: rgb(6 182 212)">
                                    ...
                                    <form action="{{ route('maps.destroy', $map->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="p-2 bg-red-500 text-white rounded-r-lg border" style="background-color: rgb(220 38 38)" onclick="return confirm('Are you sure you want to delete this map?')">
