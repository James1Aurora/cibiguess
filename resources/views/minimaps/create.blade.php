...
@section('title', 'Add Mini Map | CibiGuess')

@section('content')
    <div class="bg-cyan-600 rounded-lg p-3 w-full min-h-32 mb-3">
        <p class="text-white tracking-tighter font-semibold text-xl">Add Mini Map</p>
    </div>
    ...
    <div class="border border-gray-300 p-4 rounded-lg md:w-1/2">
        <form action="{{ route('minimaps.store') }}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="rounded-lg w-fit border border-gray-300 relative my-3">
                <img id="preview" src="#" alt="spot map" draggable="false" class="max-h-[250px]"
                    style="display: none;" />
            </div>
            ...
            <label class="form-control w-full">
                ...
                <input type="text" placeholder="Type name..." name="name"
                    class="input input-bordered w-full px-3 py-2 h-fit min-h-fit" value="{{ old('name') }}" />
            </label>
            @error('name')
                <div class="mb-1">
                    <p class="text-red-500 text-sm">{{ $message }}</p>
            ...
            <label class="form-control w-full">
                ...
                <input type="text" placeholder="Building name..." name="building"
                    class="input input-bordered w-full px-3 py-2 h-fit min-h-fit" value="{{ old('building') }}" />
            </label>
            @error('building')
                <div class="mb-1">
                    <p class="text-red-500 text-sm">{{ $message }}</p>
